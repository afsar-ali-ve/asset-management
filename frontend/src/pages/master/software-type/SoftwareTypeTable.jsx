...
import { getSoftwareTypes, deleteSoftwareType } from '../../../services/api';
const filterOptions = ['Active Software Types', 'All Software Types'];
const allColumns = [
    { field: 'display_name', label: 'Software Type' },
    { field: 'description', label: 'Description' },
    { field: 'enable_compliance', label: 'Enable Compliance' },
];
const defaultColumnWidths = {
    display_name: 260,
    description: 420,
    enable_compliance: 190,
};
const getSoftwareTypeColumnValue = (item, field) => {
    if (field === 'enable_compliance') {
        return item.enable_compliance ? 'Yes' : 'No';
    }
    return item[field] ?? '';
};
const SoftwareTypeTable = () => {
    ...
    const fetchSoftwareTypes = async () => {
        setLoading(true);
        try {
            const response = await getSoftwareTypes();
    ...
    const filteredSoftwareTypes = useMemo(() => {
        return softwareTypes.filter((item) => {
            const matchesActiveFilter = activeFilter === filterOptions[0] ? item.enable_compliance : true;
            ...
            return allColumns.every((column) => {
                ...
                return valueMatchesColumnFilter(getSoftwareTypeColumnValue(item, column.field), filter);
            });
        });
    }, [activeFilter, columnFilters, softwareTypes]);
    const sortedSoftwareTypes = useMemo(() => {
        return [...filteredSoftwareTypes].sort((a, b) => {
            const aValue = getSoftwareTypeColumnValue(a, sortField);
            const bValue = getSoftwareTypeColumnValue(b, sortField);
            const result = aValue.toString().localeCompare(bValue.toString(), undefined, { numeric: true, sensitivity: 'base' });
            return sortDirection === 'asc' ? result : -result;
        });
    }, [filteredSoftwareTypes, sortDirection, sortField]);
    const totalPages = Math.max(1, Math.ceil(sortedSoftwareTypes.length / pageSize));
    useEffect(() => {
    ...
    const paginatedData = sortedSoftwareTypes.slice(startIndex, endIndex);
    ...
    const handleDelete = (id) => {
        const item = actionRow || softwareTypes.find((currentItem) => currentItem.id === id) || { id };
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            setDeleteError('Unable to delete this software type. Please try again.');
        }
        finally {
    ...
    return (<div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6 space-y-5">
        ...
        <table className="w-full min-w-[700px] table-fixed divide-y divide-slate-200">
          <thead className="bg-slate-50">
            <tr>
              ...
              {visibleColumns.includes('display_name') && (renderColumnHeader('display_name', 'Software Type'))}
              {visibleColumns.includes('description') && (renderColumnHeader('description', 'Description'))}
              {visibleColumns.includes('enable_compliance') && (renderColumnHeader('enable_compliance', 'Enable Compliance'))}
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-slate-200">
            {paginatedData.map((item) => (<tr key={item.id} className="hover:bg-slate-50 transition-colors duration-150">
                ...
                {visibleColumns.includes('enable_compliance') && (<td className="px-4 py-4 whitespace-nowrap text-sm text-slate-900" style={getColumnStyle('enable_compliance')}>{getSoftwareTypeColumnValue(item, 'enable_compliance')}</td>)}
              </tr>))}
      ...
      <div className="mt-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3 text-sm text-slate-600">
          <span>
            {sortedSoftwareTypes.length === 0 ? '0 to 0 of 0' : `${startIndex + 1} to ${Math.min(endIndex, sortedSoftwareTypes.length)} of ${sortedSoftwareTypes.length}`}
          </span>
      ...
      <Modal
        open={open}
        title={editing ? 'Edit Software Type' : 'Add Software Type'}
        description={editing ? 'Update software type details.' : 'Create a new software type.'}
        onClose={handleClose}
        maxWidth="max-w-3xl"
      >
        <SoftwareTypeForm editing={editing} onSave={handleSave} onClose={handleClose} saving={saving}/>
      </Modal>
      <DeleteConfirmModal
        isOpen={Boolean(deleteTarget)}
        title="Delete Software Type"
        message={deleteError || 'Are you sure you want to delete this software type?'}
...
export default SoftwareTypeTable;
